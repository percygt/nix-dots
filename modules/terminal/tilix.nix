  ...
  config = lib.mkMerge [
    (lib.mkIf (desktop == "sway") {
      wayland.windowManager.sway.config.keybindings = {
        "Alt+return" = "exec ${lib.getExe g.terminal.xfce4-terminal.package}";
      };
    })
    (lib.mkIf config.modules.terminal.tilix.enable {
      home.packages = [ g.terminal.tilix.package ];
      wayland.windowManager.sway.config.keybindings = {
        "Ctrl+return" = "exec ${lib.getExe g.terminal.tilix.package}";
      };
      dconf.settings = {
        "com/gexperts/Tilix" = {
