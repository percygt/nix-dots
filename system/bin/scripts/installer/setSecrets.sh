...
if [ ! -f "$HOME"/secrets_updated ]; then
	...
	yq ".keys[.keys[] | select(anchor == \"$TARGET_HOST\") | path | .[-1]] = \"$SYSTEM_AGE_PUBLICKEY\"" -i "$SECRETS_DIR/.sops.yaml"
	yq ".keys[.keys[] | select(anchor == \"$TARGET_HOST\") | path | .[-1]] = \"$HOME_AGE_PUBLICKEY\"" -i "$SECRETS_DIR/.sops.yaml"
