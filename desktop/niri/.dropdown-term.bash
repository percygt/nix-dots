...
ACTIVE_WORKSPACE_IDX=$(niri msg --json workspaces | jq -r ".[] | select(.is_focused==true) | .idx") || notify "ndrop: Error executing dependencies 'niri msg' or 'jq'" "Check terminal output of 'ndrop $COMMANDLINE'"
...
if [[ -n $(niri msg --json windows | jq -r ".[] | select(.app_id==\"$CLASS\" and .workspace_id!=$ACTIVE_WORKSPACE)") ]]; then
	if [[ $FOCUS == false ]]; then
		...
		niri msg action move-window-to-workspace --window-id "$WINDOW_ID" "$ACTIVE_WORKSPACE_IDX" || notify "ndrop: Error moving '$COMMANDLINE' to current workspace"
	...
	if [[ $FOCUS == false ]]; then
		...
		niri msg action move-window-to-workspace --focus false --window-id "$WINDOW_ID" "ndrop" || notify "ndrop: Error moving '$COMMANDLINE' to workspace 'ndrop'"
